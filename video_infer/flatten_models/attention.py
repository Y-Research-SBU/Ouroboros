...
class Transformer3DModel(ModelMixin, ConfigMixin):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, timestep=None, return_dict: bool = True, \
        ...
        # print("attention hidden0: ", torch.isnan(hidden_states).any())
        # Blocks
        for block in self.transformer_blocks:
            ...
            # print("attention hidden1: ", torch.isnan(hidden_states).any())
        # Output
        if not self.use_linear_projection:
            hidden_states = (
        ...
        # print("attention hidden2: ", torch.isnan(hidden_states).any())
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, timestep=None, attention_mask=None, video_length=None, \
        ...
        # print("basic transformer block0: ", torch.isnan(hidden_states).any())
        ...
        # print("basic transformer block1: ", torch.isnan(hidden_states).any())
        ...
        # print("basic transformer block2: ", torch.isnan(hidden_states).any())
        # Feed-forward
        hidden_states = self.ff(self.norm3(hidden_states)) + hidden_states

        # print("basic transformer block3: ", torch.isnan(hidden_states).any())
...
class FullyFrameAttention(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, video_length=None, inter_frame=False, **kwargs):
        ...
        # print("fully attention0, Hidden states after group norm:", hidden_states.min(), hidden_states.max(), torch.isnan(hidden_states).any())
        ...
        # print("fully attention1: ", torch.isnan(hidden_states).any())
        ...
        if False:
            ...
            # print("fully attention1.5: ", torch.isnan(hidden_states).any())
            ...
            # print("fully attention1.6: ", torch.isnan(query).any(), torch.isnan(key).any(), torch.isnan(value).any())
            ...
            # print("fully attention1.7: ", torch.isnan(traj).any(), torch.isnan(mask).any())
            ...
            # print("fully attention1.8: ", torch.isnan(t_inds).any(), torch.isnan(x_inds).any(), torch.isnan(y_inds).any())
            # print("Indices:", t_inds.min(), t_inds.max(), x_inds.min(), x_inds.max(), y_inds.min(), y_inds.max())
            ...
            # print("fully attention1.9: ", torch.isnan(query_tempo).any(), torch.isnan(key_tempo).any(), torch.isnan(value_tempo).any())
            # print("Query Tempo Min/Max:", query_tempo.min(), query_tempo.max(), torch.isnan(query_tempo).any())
            # print("Key Tempo Min/Max:", key_tempo.min(), key_tempo.max(), torch.isnan(key_tempo).any())

            # mask = rearrange(torch.stack([mask, mask]),  'b f n l -> (b f) n l')
            mask = rearrange(torch.stack([mask]),  'b f n l -> (b f) n l')
            ...
            # print("fully attention1.95: ", torch.isnan(attn_bias).any())
            ...
            # print("fully attention1.96: ", torch.isnan(query_tempo).any(), torch.isnan(key_tempo).any(), torch.isnan(value_tempo).any())
            # print(query_tempo.size(), key_tempo.size(), value_tempo.size())
            ...
            # CHANGE: NORMALIZE
            # # Temporary normalization for stability
            # query_tempo_norm = query_tempo / query_tempo.norm(dim=-1, keepdim=True)
            # key_tempo_norm = key_tempo / key_tempo.norm(dim=-1, keepdim=True)

            # # Attention calculation
            # d_k = query_tempo.size(-1)
            # attn_matrix2 = (query_tempo_norm @ key_tempo_norm.transpose(-2, -1)) / math.sqrt(d_k) + attn_bias

            # print("fully attention1.961: ", torch.isnan(query_tempo @ key_tempo.transpose(-2, -1)).any())
            # print("fully attention1.962: ", math.sqrt(query_tempo.size(-1)))
            # print("fully attention1.963: ", torch.isnan(query_tempo @ key_tempo.transpose(-2, -1) / math.sqrt(query_tempo.size(-1))).any())
            # print("fully attention1.96301: ", torch.isinf(query_tempo @ key_tempo.transpose(-2, -1) / math.sqrt(query_tempo.size(-1))).any())
            # print("fully attention1.9631: ", torch.isnan(attn_bias).any())
            # print("fully attention1.96311: ", torch.isinf(attn_bias).any())
            # print("fully attention1.964: ", torch.isnan(attn_matrix2).any())

            # if torch.isinf(query_tempo @ key_tempo.transpose(-2, -1) / math.sqrt(query_tempo.size(-1))).any():
            #     torch.save(query_tempo, "/home/yifengx4/3d-gen/query_tempo.pt")
            #     torch.save(key_tempo, "/home/yifengx4/3d-gen/key_tempo.pt")
            #     # print(a)

            # if torch.isnan(attn_matrix2).any():
            #     torch.save(query_tempo @ key_tempo.transpose(-2, -1) / math.sqrt(query_tempo.size(-1)), "attn_matrix2_before_bias.pt")
            #     torch.save(attn_bias, "attn_bias.pt")
            #     # print(a)

            attn_matrix2 = F.softmax(attn_matrix2, dim=-1)
            out = (attn_matrix2@value_tempo).squeeze(-2)

            # print("fully attention1.965: ", torch.isnan(attn_matrix2).any())

            # print("fully attention1.97: ", torch.isnan(out).any())

            hidden_states = rearrange(out,'(b f) k (h w) d -> b (f h w) (k d)', b=int(batch_size/video_length), f=video_length, h=h, w=w)

        # print("fully attention2: ", torch.isnan(hidden_states).any())
