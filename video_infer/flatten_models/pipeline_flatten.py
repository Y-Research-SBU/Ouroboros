...
class FlattenPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        #         latent_model_input = torch.cat([latent_model_input, torch.cat([cond_latents] * 2)], dim=1) if do_classifier_free_guidance else torch.cat([latent_model_input, cond_latents], dim=1)
        ...
        # latents_common = torch.randn(
        #     (cond_latents.shape[0], cond_latents.shape[1], 1, cond_latents.shape[3], cond_latents.shape[4]), generator=generator, dtype=cond_latents.dtype, device=device
        # )
        
        # # LOTUS
        # latents = []
        # last_frame_latent = None
        # for i in range(cond_latents.shape[2]):
        #     _latents = latents_common
        #     if last_frame_latent is not None:
        #         _latents = 0.9 * _latents + 0.1 * last_frame_latent
        #     latents.append(_latents)
        #     last_frame_latent = cond_latents[:,:,i].unsqueeze(2)

        # latents = torch.cat(latents, dim=2)

        latents = torch.rand_like(cond_latents, dtype=cond_latents.dtype, device=device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                print("input: ", torch.isnan(latent_model_input).any())
                ...
                print("noise_pred: ", noise_pred[0][0][0][0][0])
