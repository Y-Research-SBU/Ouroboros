...
class VaeImageProcrssorAOV(VaeImageProcessor):
    ...
    def postprocess(
        ...
        origin_shape: Optional[List[int]] = None,
    ):
        ...
        if origin_shape is not None:
            if list(image.shape[-2:]) != origin_shape:
                image = TF.resize(image, size=origin_shape, antialias=False)
...
class StableDiffusionAOVMatEstPipeline(
    ...
    def prepare_video_latents(self, frames, batch_size, dtype, device, do_classifier_free_guidance=False, generator=None):
        ...
        if do_classifier_free_guidance:
            uncond_video_latents = torch.zeros_like(latents)
            latents = torch.cat(
                [latents, latents, uncond_video_latents], dim=0
            )

        return latents[None]
    
    def decode_latents(self, latents, required_aovs, output_type, origin_shape):
    ...
    def __call__(
        ...
        for photo in photos:
            ...
            frames.append(preprocessed_photo) # create frame dimension
        ...
        video_latents = self.prepare_video_latents(frames, batch_size, 
                                                   self.unet.dtype, device, 
                                                   do_classifier_free_guidance, generator=generator)
        
        num_frames, height, width = video_latents.shape[-3:]
        ...
        num_channels_image = video_latents.shape[1]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # concat latents, video_latents in the channel dimension
                scaled_latent_model_input = self.scheduler.scale_model_input(
                    latent_model_input, t
                )
                scaled_latent_model_input = torch.cat(
                    [scaled_latent_model_input, video_latents], dim=1
                )
            ...
            # 10. Decode latents
            video = self.decode_latents(latents, required_aovs, output_type, origin_shape)
        ...
        return StableDiffusionAOVPipelineOutput(images=video)
